...
- 状態: **廃止**（2026-08-24、[ADR 0005](0005-release-build-off-repository.md) が置き換え）
- 日付: 2026-08-03
...
この ADR が想定した dev コンテナ経路は、**配布先ホストで一度も定常運用に
乗りませんでした。**前提としていた配布ディレクトリはホスト上に作られておらず、
ビルドを委ねるはずの dev コンテナも停止したままです。

その間に、この ADR が「置き換えではない」と書いていた CI 経路のほうが二度
入れ替わりました。当時の `.github/workflows/build.yml` は削除され、リリース
ビルドはビルドホスト側の 3 段構成（ビルド／署名／検証・保管）へ移り、配布は
専用の配布サイトが担うようになっています。この ADR が解こうとした
「配布先ホストに Flutter を置かずに配布物を用意する」という問題は、
**そちらが先に解いていました。**

したがって `scripts/build-remote-container.sh` と
`scripts/build-remote-container.env.example` は削除しました。

残っている決定は 1 つだけで、それは今も有効です。

- **`scripts/build.sh`** は残します。Flutter がある場所ならどこでも同じように
  動き、`dist/` に manifest 付きの配布物一式を書き出します。CI を通さずに
  任意のブランチを手元で焼く経路として、依然として使えます。

「なぜ生成物をビルド後に戻すのか」で述べた理由も生きています。
`lib/shared/build_info.dart` を書き換えたまま終わらないという性質は、
`scripts/check_release_contract.sh` にも引き継いであります。

置き換え後の設計は [ADR 0005](0005-release-build-off-repository.md) を参照。
